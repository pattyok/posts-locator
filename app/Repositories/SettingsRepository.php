...
class SettingsRepository
{
	...
	*/
	public function defaultMap($return = 'latitude')
	{
		...
		if ( $return == 'user_location' ){
			return ( isset($option['load_locations']) && $option['load_locations'] == 'true' ) ? 'true' : 'false';
		}
	}
	...
	*/
	 ...
	 * Get Map Pin Field
	 */
	public function getMapPinField()
	{
		return get_option('wpsl_map_pin_field');
	}

	/**
	 * Get Map Clustering Info
	 */
	public function getMapClustering()
	{
		$clustering = get_option('wpsl_map_pin_cluster');
		if ('true' == $clustering) {
			if (!empty(get_option('wpsl_map_pin_cluster_directory'))) {
				$directory = get_option('wpsl_map_pin_cluster_directory');
			} else {
				$directory = \SimpleLocator\Helpers::plugin_url() . '/assets/images/markerclusters/m';
			}
			return $directory;
		} else {
			return "false";
