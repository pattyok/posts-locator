...
jQuery(function ($) {
   ...
   */
	...
	function setFormElements(form) {
		...
		if ($(active_form).siblings("#widget").length < 1) {
			...
			if (wpsl_locator_options.wrapper.charAt(0) === ".") {
				var wrapper = $(form).find(wpsl_locator_options.wrapper);
			} else {
				var wrapper = $(wpsl_locator_options.wrapper);
			}
		} else {
			// Its the widget
			...
			var wrapper = $(active_form).find(wrapper);
		}
		var formelements = {
			parentdiv: $(form),
   ...
   */
	function submitForm(form, filters, submit_success, submit_error) {
		...
		$(formelements.wrapper).addClass("loading");
		...
		if (submit_success == null) {
			submit_success = function (data, form) {
				...
				$(formelements.wrapper).removeClass("loading");
			};
		}
   ...
   */
	...
	function handleParams(guid) {
		var form = $(".simple-locator-form[data-guid='" + guid + "']");
		var u = new Url;
		var filterList = {};
		if (u.queryLength() > 0) {
